...
class AwalModel extends Model
{
    public static function getuserData($username,$password){
        ...
        $sql = 'SELECT id,username,nama_lengkap,tipe,level,nm_level,aktif,email,password,PASSWORD("'.$password.'") as password_input
                FROM
                (
                    SELECT us.id,us.username,us.nama_lengkap,us.tipe,us.level,lvl.level as nm_level,us.email,us.password,
                    (
                        SELECT aktif
                        FROM tbl_sdm
    ...
    public static function getuserData_parent($username,$username_siswa,$password){
        // $value=DB::table('tbl_users')->where('email', $username)->where('blokir','N')->first();
        $sql = 'SELECT id,username,nama_lengkap,tipe,level,nm_level,aktif,email,password,PASSWORD("'.$password.'") as password_input
                FROM
                (
                    ...
                    (
                        SELECT aktif
                        FROM tbl_sdm
                        WHERE finger = "'.$username_siswa.'"
                        UNION ALL
                        SELECT aktif
                        FROM tbl_siswa
                        WHERE nim = "'.$username_siswa.'"
                    ) as aktif
                    from tbl_users us
                    LEFT JOIN tbl_user_level lvl on us.level=lvl.id_user_level
                    WHERE us.username ="'.$username.'"
